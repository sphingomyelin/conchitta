...
const float EXPOSURE_TIME_CHOOSEN = 0.2;//0.5
...
int ledBrightness;
 ...
 const int THRESHOLD_BRIGHTNESS = 30;//25;
...
int initComm(){
  ...
  if((fd = serialOpen(device,baud))<0){
    cout<<"ERROR: trouble opening serial port "<<device<<endl;
    return (-1);
  }
  else cout<<"serialOpen OK"<<endl;

  if(wiringPiSetup() == -1){
    cout<<"ERROR: unable to start wiringPi"<<endl;
    return(-1);
  }
...
int commArduinoReceive(){
  ...
  if(valueAvailable = serialDataAvail(fd)){
    cout<<"BUFFER:value of available: "<< valueAvailable<<endl;
    for(int i=0; i < valueAvailable-1; i++ ) {
	serialGetchar(fd);
    }
...
int intToChar(){
  ...
  else if(state == BOTTLE_DETECTION){
    letter = X;
	///maximum of position_object
    for(int k=0;k<MAX_OBJ;k++){
      if(maxYvalue < position_objects[k][Y]){
        maxYvalue = position_objects[k][Y]; 
        obj_index = k;
      }
    }
    ///no position object -> no send data  
    if(maxYvalue<0){
      return -1;
    }	
  }
  ...
  for(int j=0;j<NB_CHAR;j++){
	...
	else if(letter == BRIGHTC){
		charvalue[BRIGHTC][0]='h';
		object = BRIGHTC;
		value = ledBrightness;
		j = NB_CHAR;
	}
  ...
  for(int i=0; i<NB_CHAR;i++){
    cout<<"charvalue XYC"<<i+1<<": "<<charvalue[i]<<endl;// TEST
  }
...
int commArduinoSend(){
  ...
  if(state == GO_HOME){
	...
	for(int i=0; i<2;i++){
		serialPuts(fd,charvalue[COLOR+i]);
	}
	double t1 = ((double)getTickCount()-t)/getTickFrequency();
	cout<<"TotTimeSL: "<<t1<<endl;
  }
  else if (state == BOTTLE_DETECTION){
	double t = (double)getTickCount();
	  for(int i=0; i<2;i++){
...
void findColor(int meanpixValue){
  if(meanpixValue> MINTHRES_Y && meanpixValue < MAXTHRES_Y){
    ...
    cout<<"color: range not defined"<<endl; // TEST
    color = 'n';
  }
}
...
int init_camera(){
  ...
  if (!Camera.open()) {
    cerr<<"ERROR: opening the camera failed"<<endl;
    return -1;
  }
